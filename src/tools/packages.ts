...
interface CatalogPackage {
  ...
  changelog?: string;
  /** Display name — production-verified 2026-07-01: this API names it `dname`, not
   *  `name` (that's `SYNO.Core.Package.list`'s field, a different endpoint). */
  dname?: string;
  /** Publisher — production-verified: `maintainer`, not `publisher`. */
  maintainer?: string;
  /** Description — production-verified: `desc`, not `description`. */
  desc?: string;
  /** Dependency map (`{pkgId: versionConstraint}` or null) — production-verified:
